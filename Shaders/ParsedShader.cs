    ...
    internal static class ParsedShader {
        ...
        static ParsedShader () {
            foreach (var k in Enum.GetValues<VariableKind>())
                _EH[(int)k] = typeof(Calls).GetField($"glGet{k}Location", BindingFlags.Static | BindingFlags.Public).GetValue(null) is glGetLocation getLoc ? getLoc : throw new Exception();
        }
        internal static void Prepare (Type type) {
            ...
            var nameProperty = type.GetProperty("Id", BindingFlags.Static | BindingFlags.Public) ?? throw new Exception("no property 'Id'");
            var nameBackingField = GetBackingField(type, nameProperty, BindingFlags.Static) ?? throw new Exception("no backing field for Id");
            ...
            foreach (var prop in type.GetProperties(BindingFlags.Public | BindingFlags.Static))
                if (GetAttribute<GlAttribAttribute>(prop, false) is GlAttribAttribute attr) {
                    var fi = GetBackingField(type, prop, BindingFlags.Static) ?? throw new Exception($"no backing field for {prop.Name} of {typeName}");
