namespace Engine {
    ...
    class GlWindowBase:IDisposable {
        ...
        private static void SetHintsFrom (string filepath) {
            ...
            foreach (var line in File.ReadAllLines(filepath))
                if (hintRegex.TryMatch(line, out var m) && Enum.TryParse<Hint>(m.Groups[1].Value, true, out var hint))
                    Glfw.WindowHint(hint, m.Groups[2].Value == "true");
        }
        ...
        protected Camera Camera { get; } = new Camera(new(-10f, 0f, -1f));
        ...
        private readonly float[] xcoords = new float[_SAMPLES], ycoords = new float[_SAMPLES];
...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        private void OnFramebufferSizeInternal (IntPtr _, int width, int height) {
        ...
        private void HandleDebug (int source, int type, uint id, int severity, int length, string message, IntPtr _) => throw new ApplicationException(string.Format(_DEBSTR, source, type, severity, message));
